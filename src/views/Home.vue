...
import ToolkitModal from '@/components/ToolkitModal.vue' // 🔥 引入新组件

const showToolkitModal = ref(false) // 🔥 新增状态
...
const shortcutLinks = [
    ...
    { name: 'YouTube', url: 'https://youtube.com', viewBox: '0 0 1024 1024', paths: [{ d: 'M426.666667 682.666667V384l256 149.845333L426.666667 682.666667z m587.093333-355.541334s-10.026667-71.04-40.704-102.357333c-38.954667-41.088-82.602667-41.258667-102.613333-43.648C727.168 170.666667 512.213333 170.666667 512.213333 170.666667h-0.426666s-214.954667 0-358.229334 10.453333c-20.053333 2.389333-63.658667 2.56-102.656 43.648-30.677333 31.317333-40.661333 102.4-40.661333 102.4S0 410.538667 0 493.952v78.293333c0 83.456 10.24 166.912 10.24 166.912s9.984 71.04 40.661333 102.357334c38.997333 41.088 90.154667 39.765333 112.938667 44.074666C245.76 893.568 512 896 512 896s215.168-0.341333 358.442667-10.752c20.053333-2.432 63.658667-2.602667 102.613333-43.690667 30.72-31.317333 40.704-102.4 40.704-102.4s10.24-83.413333 10.24-166.869333v-78.250667c0-83.456-10.24-166.912-10.24-166.912z', fill: '#FF0000' }] },
    {
        ...
        paths: [
            ...
            {
                d: 'M786.432 238.592m-61.44 0a61.44 61.44 0 1 0 122.88 0 61.44 61.44 0 1 0-122.88 0Z',
                fill: '#d3217b'
            },
            {
                d: 'M512 248.832A263.168 263.168 0 1 0 776.192 512 263.168 263.168 0 0 0 512 248.832z m0 433.664A170.496 170.496 0 1 1 683.52 512 170.496 170.496 0 0 1 512 682.496z',
                fill: '#d3217b'
            }
        ]
    },
...
<template>
    <div class="home-container">
        <div class="wallpaper-overlay-mask" :class="{ 'active': wallpaperMask }"></div>
        <!-- 🔥 壁纸切换指示器 -->
        ...
        <div class="hero-section">
            ...
            <div class="search-section">
                    ...
                    <transition name="fade">
                        <div v-if="showEngineDropdown" class="engine-dropdown-menu" @click.stop>
                            <div v-for="engine in searchEngines" :key="engine.name" class="dropdown-item"
                                ...
                                <svg class="dropdown-icon-svg" :viewBox="engine.viewBox || '0 0 24 24'"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path :d="engine.iconPath" fill="currentColor" />
                                </svg>
                ...
                <div class="shortcut-dock animate__animated animate__fadeInUp animate__delay-1s">
                    <component :is="link.isAction ? 'div' : 'a'" v-for="link in shortcutLinks" :key="link.name"
                        :href="link.isAction ? undefined : link.url" class="shortcut-icon-btn" :title="link.name"
                        :target="link.isAction ? undefined : '_blank'" rel="noopener noreferrer"
                        @click="handleShortcutClick(link)">
        ...
        <!-- 快捷链接 "更多" 弹框 -->
        <transition name="modal-fade">
            <ToolkitModal v-if="showToolkitModal" @close="showToolkitModal = false" />
        </transition>
    </div>
</template>
...
.glass-search-box.dropdown-open {
    /* 只要比下面的 shortcut-dock (z-index: 10) 高即可。
       设置为 20 或更高，确保它浮在社交图标之上。
    */
    z-index: 20; 
}
...
/* 深色模式适配 (如果您的全局支持 dark class) */
:global(html.dark) .engine-dropdown-menu {
...
:global(html.dark) .engine-dropdown-menu::before {
...
/* 选中状态高亮图标 */
.dropdown-item.selected .dropdown-icon-svg {
    opacity: 1;
    fill: #42b883;
}
...
@media (max-width: 768px) {
    ...
    /* 3. 搜索框下面的间距调整，给两行图标留位置 */
    .search-section {
        gap: 25px;
        /* 搜索框和图标栏之间的距离 */
    }

    /* 🔥🔥🔥 修复设置按钮位置：把它抬高，避开底部导航栏 🔥🔥🔥 */
    .settings-btn {
        /* 强制覆盖默认的 bottom: 30px */
...
/* 1. 去掉安卓/iOS浏览器默认的点击高亮背景色 (那个蓝色的框) */
* {
...
/* 2. 快捷图标 (GitHub/X 等)：点击时缩小并高亮 */
.shortcut-icon-btn:active {
...
/* 4. 搜索引擎切换图标 (Bing/Google 图标)：点击缩放 */
