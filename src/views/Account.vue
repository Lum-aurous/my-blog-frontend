...
const user = ref({
    ...
    bio: '',
    social_link: '',
    social_links_list: [''] // 🆕 新增：前端专用的数组，默认给一个空位
})
...
const fetchUserInfo = async () => {
    ...
    try {
        ...
        if (responseData.success && responseData.data) {
            ...
            // 🔥🔥🔥 解析社交链接逻辑 🔥🔥🔥
            try {
                const rawLink = dbUser.social_link;
                if (rawLink) {
                    // 尝试解析是否为 JSON 数组字符串 (例如 '["url1", "url2"]')
                    if (rawLink.startsWith('[') && rawLink.endsWith(']')) {
                        user.value.social_links_list = JSON.parse(rawLink);
                    } else {
                        // 如果是旧数据（普通字符串），直接放进数组第一个
                        user.value.social_links_list = [rawLink];
                    }
                } else {
                    user.value.social_links_list = ['']; // 默认给一个空行
                }
            } catch (e) {
                console.error('解析社交链接失败', e);
                user.value.social_links_list = [''];
            }
            ...
            // 🔥🔥🔥【新增】同步提取用户的统计数据 (逻辑参考 Blog.vue)
            if (dbUser.stats) {
                userStats.value = {
                    // 后端通常返回 originalCount 代表原创文章数，这里映射为内容数
                    articleCount: dbUser.stats.originalCount || 0,
                    categoryCount: dbUser.stats.categoryCount || 0,
                    totalViews: dbUser.stats.totalViews || 0
                }
                console.log('📊 个人统计数据已更新:', userStats.value)
            }
...
const validateForm = () => {
    ...
    // 5. 网址验证
    user.value.social_links_list.forEach((link, index) => {
        if (link && link.trim() !== '') {
            try {
                new URL(link); // 验证单个链接是否合法
            } catch {
                errors.push(`第 ${index + 1} 个社交链接格式不正确，请检查`);
            }
        }
    });
...
const handlePublish = async () => {
    ...
    // 🔥 1. 先处理社交链接数组 -> 转为 JSON 字符串
    // 过滤掉空链接
    const validLinks = user.value.social_links_list.filter(link => link && link.trim() !== '');
    // 如果有有效链接，转为 JSON 字符串；否则为 null
    user.value.social_link = validLinks.length > 0 ? JSON.stringify(validLinks) : null;
...
// ➕ 添加链接输入框
const addSocialLink = () => {
    if (user.value.social_links_list.length >= 5) {
        message.warning('最多添加 5 个社交链接哦');
        return;
    }
    user.value.social_links_list.push('');
}

// ➖ 删除链接输入框
const removeSocialLink = (index) => {
    user.value.social_links_list.splice(index, 1);
    // 如果全删光了，留一个空的
    if (user.value.social_links_list.length === 0) {
        user.value.social_links_list.push('');
    }
}
...
// 🔥🔥🔥【新增】存储当前用户的个人统计数据 (对应 Blog.vue 的 userPersonalStats)
const userStats = ref({
    articleCount: 0,
    categoryCount: 0,
    totalViews: 0
...
// 1. 定义默认背景图 (和 Blog.vue 保持一致)
const defaultBanner = 'https://w.wallhaven.cc/full/5g/wallhaven-5gjgj8.jpg'

// 2. 图片路径清洗工具 (复用昨天的核心逻辑)
...
// 3. 动态计算 Banner 地址 (监听 userStore)
const bannerSrc = computed(() => {
    // 优先使用 store 中的最新 banner，如果没有则自动回退到 defaultBanner
    return getProxyUrl(userStore.user?.banner)
})
            ...
            <main class="content">
                <div v-if="activeTab === 'personal'" class="panel">
                    ...
                    <div class="form-group">
                        <label class="label">
                            主页链接 / 社交媒体
                            <span v-if="user.social_links_list.length > 1" class="badge-count">
                                +{{ user.social_links_list.length - 1 }} 更多
                            </span>
                        </label>

                        <div class="input-with-icon">
                            <span class="input-prefix">🔗</span>
                            <input type="url" v-model="user.social_links_list[0]" class="input"
                                placeholder="请输入您的主页链接 (https://...)" />
                        </div>

                        <p class="tip" style="margin-top: 8px; color: rgba(255,255,255,0.5);">
                            💡 想要查看更多链接？请前往
                            <span @click="activeTab = 'people'" class="link-jump">“用户与分享”</span>
                            面板配置您的社交矩阵。
                        </p>
                    </div>
                        ...
                        <div class="card-preview-container">
                            <div class="sidebar-card profile-card-crystal mini-preview">
                                <div class="profile-bg-illustration">
                                    <img :src="bannerSrc" class="illus-img" alt="bg">
                                </div>
                                ...
                                <div class="profile-stats-grid">
                                    <div class="stat-col">
                                        ...
                                        <div class="stat-num">{{ userStats.articleCount }}</div>
                                    </div>
                                    ...
                                    <div class="stat-col">
                                        ...
                                        <div class="stat-num">{{ userStats.categoryCount }}</div>
                                    </div>
                                    ...
                                    <div class="stat-col">
                                        ...
                                        <div class="stat-num">{{ userStats.totalViews }}</div>
                                    </div>
                                </div>
                    ...
                    <div class="share-section">
                        ...
                        <div class="social-input-group">
                            <div class="group-header-row">
                                <label class="label" style="margin-bottom:0">🔗 社交矩阵</label>
                                <button class="add-link-btn" @click="addSocialLink"
                                    v-if="user.social_links_list.length < 5">
                                    + 添加链接
                                </button>
                            </div>

                            <div class="social-list-editor">
                                <div v-for="(link, index) in user.social_links_list" :key="index"
                                    class="input-with-icon multi-row">
                                    <span class="index-badge">#{{ index + 1 }}</span>
                                    <input type="url" v-model="user.social_links_list[index]" class="input"
                                        placeholder="https://..." />
                                    <button class="remove-btn" @click="removeSocialLink(index)"
                                        v-if="user.social_links_list.length > 1">
                                        ✕
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="social-actions-row">
                                <span class="status-hint">
                                    <i class="icon-info">ℹ️</i> 已展示到主页...
                                </span>
                                <button @click="handlePublish" class="btn-primary-sm" :disabled="isSaving">
                                    {{ isSaving ? '同步中...' : '同步' }}
                                </button>
                            </div>
                        </div>
...
@media (max-width: 768px) {
    .account-container {
        /* 移除固定高度和 overflow hidden，允许页面自然滚动 */
        height: auto;
        min-height: 100vh;
        overflow-y: auto;
        display: block;
